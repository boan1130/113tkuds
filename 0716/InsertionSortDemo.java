...
/*基本插入排序
     * Time Complexity: O(n²)
     * 說明：外層迴圈執行 n-1 次，內層 while 迴圈最多執行 i 次
     * 總比較次數為 1 + 2 + ... + (n-1) = n(n-1)/2 = O(n²)
     * 最佳情況下已排序陣列只需 n-1 次比較，達到 O(n)
     */
    /**
     * 插入排序實作
     * 時間複雜度：最佳 O(n)，最壞 O(n²)，平均 O(n²)
     ...
     */
     ...
     /*=二分插入排序
     * Time Complexity: O(n²)
     * 說明：雖然使用二分搜尋找插入位置只需 O(log n)，但移動元素仍需 O(n)
     * 外層迴圈 n-1 次，每次移動平均 n/2 個元素，總時間複雜度為 O(n²)
     * 比較次數降為 O(n log n)，但移動次數仍為 O(n²)
     */